...
### Fixed
- 프롬프트 편집 시 내용에서 모든 변수를 제거해도 마지막 변수가 UI 및 데이터베이스에서 올바르게 삭제되지 않던 문제 해결 (백엔드 API 및 프론트엔드 모달 로직 수정)
- 폴더 삭제 시 해당 폴더 내의 프롬프트들이 함께 삭제되지 않고孤立되던 오류 수정
- 프롬프트 상세 정보 모달에서 변수의 기본값 편집 시, 텍스트 에디터에 이전에 사용자가 입력했을 수 있는 값이 아닌 변수의 실제 저장된 기본값이 표시되도록 수정
- 프롬프트 상세 정보 모달에서 변수 기본값 저장 로직의 안정성 강화

### Improved
- 프롬프트 추가/편집 모달 (`UserPromptEditModal`, `PromptAddEditModal`)에서 변수 관리 방식을 전면 개편:
    - 이제 변수는 프롬프트 내용(예: `{{variable}}`)에 의해서만 자동으로 인식되고 관리됩니다.
    - 수동으로 변수를 추가, 삭제, 수정하는 UI 및 관련 로직 제거.
    - 변수 목록은 읽기 전용으로 표시되어 사용자 경험을 간소화하고, 내용과 변수 간의 불일치로 인한 오류 가능성 감소.
- 폴더 목록에 각 폴더별로 포함된 프롬프트의 개수가 정확하게 표시되도록 개선
...
- 휴지통 기능 추가
  - 삭제된 프롬프트 목록 조회 기능
  - 프롬프트 복구 기능 (복구 시 기본 폴더로 자동 이동)
  - 프롬프트 영구 삭제 기능
  - 다중 선택 일괄 복구/영구 삭제 기능
  - 삭제된 프롬프트 상세 정보 확인 및 관리 기능
- 기본 폴더 기능 추가
  - 삭제된 프롬프트 복구 시 기본 폴더로 이동
  - 폴더 구조에서 기본 폴더 우선 표시 (모든 프롬프트와 즐겨찾기 다음에 위치)
  - 기본 폴더가 없을 경우 자동 생성 기능
- 데이터베이스 스키마 업데이트
  - `deletedAt` 및 `isDeleted` 필드 추가로 소프트 삭제 지원
  - 기본 폴더 식별을 위한 `isDefault` 필드 추가

### Improved
- React Router 기반 라우팅 시스템으로 전환
  - 페이지 간 이동 방식 개선 (URL 기반)
  - 모달을 모든 페이지에서 일관되게 표시
  - 폴더 클릭 시 라우팅 처리 개선
  - 브라우저 히스토리 관리 및 북마크 지원
- 사이드바 메뉴 구조 개선
  - 휴지통 메뉴 추가 ('삭제된 프롬프트' 섹션)
  - 태그 섹션 위치 조정
  - 메뉴 아이템 스타일 통일 및 아이콘 추가
- 프롬프트 관리 워크플로우 개선
  - 삭제 전 확인 다이얼로그 개선 (휴지통으로 이동 안내)
  - 복구 및 영구 삭제 작업 진행 상태 표시
  - 일괄 작업 시 진행 상황 시각화

### Fixed
- 폴더 전환 오류 해결
  - 다른 페이지에서 폴더 클릭 시 메인 페이지로 이동 처리
  - useNavigate를 활용한 프로그래밍 방식 라우팅 구현
  - 새로고침 후에도 현재 선택된 폴더 상태 유지
- 데이터베이스 무결성 강화
  - 프롬프트 삭제 시 관련 데이터 정합성 유지
  - 복구 작업 중 발생할 수 있는 데이터 충돌 방지 로직 추가
  - 기본 폴더 생성 및 관리 시 발생하던 동기화 문제 해결

## [v1.63] - 2025-04-13

### 수정 경로
1. 경로1: 홈 화면 > 일반(부모) 프롬프트 카드 마우스 호버시 프롬프트 편집 아이콘 클릭시
2. 경로2: 홈 화면 > 일반(부모) 프롬프트 카드 > 상세내용 모달에서 우측 상단의 편집 아이콘 클릭시
3. 경로3: 홈 화면 > 일반(부모) 프롬프트 카드 > 상세내용 모달 > 사용자 추가 탭 - 프롬프트 카드 마우스 호버시 프롬프트 편집 아이콘 클릭시
4. 경로4: 홈 화면 > 일반(부모) 프롬프트 카드 > 상세내용 모달 > 사용자 추가 탭 - 프롬프트 카드 > 상세내용 오버레이 모달 우측 상단의 편집 아이콘 클릭시

### Added
- 프롬프트 편집 중 변경 사항 미저장 시 확인 팝업 기능 추가 (`UnsavedChangesPopup`)
- 상세 모달에서 편집 버튼 클릭 시 오버레이 형태로 편집 모달 표시 기능 추가

### Improved
- 프롬프트 편집창 '변수 미리보기' 영역 스타일 개선 (고정 높이, 스크롤, 크기 조정, 패딩 등)
- 모든 프롬프트 편집 경로(1, 2, 3, 4)에서 편집 모달 닫기 로직 일관성 확보
  - 저장 성공 시 모달 자동 닫기
  - 외부 클릭 및 ESC 키 입력 시 변경 사항 확인 후 닫기
- 프롬프트 편집 모달의 버튼 텍스트 '저장'으로 통일

### Fixed
- 특정 경로(1, 2)에서 프롬프트 편집창 '변수 미리보기' 영역 크기 조정 핸들이 작게 보이던 문제 완화 (하단 여백 추가)
- 저장 후 편집 모달이 닫히지 않던 문제 해결
...
- 홈 화면 프롬프트 카드 다중 선택 기능 추가
  - 체크박스 UI 구현으로 직관적인 다중 선택 지원
  - 선택 항목 기반 일괄 작업 (삭제, 폴더 이동) 인터페이스 구현
  - 전체 선택/해제 기능 추가

### Improved
- 프롬프트 카드 컴포넌트 구조 개선
  - 기본 프롬프트 카드 컴포넌트 분리 및 확장
  - 재사용성 향상 및 일관된 UI 경험 제공
  - 코드 중복 제거 및 유지보수성 향상

### Fixed
- 다중 선택 모드에서 UI 일관성 유지 문제 해결
  - 그리드 및 리스트 뷰 간 동일한 다중 선택 경험 제공
  - 선택 상태 시각적 피드백 일관성 확보
  - 다중 선택 모드 취소 시 선택 항목 초기화 기능 추가

## [v1.51] - 2025-04-11

### Added
- 기본 프롬프트 모달에서 확장 아이콘 고정 적용
- 프롬프트 필드 하이라이트 적용
  - 기본 프롬프트 필드 하이라이트 추가
  - 사용자 추가 탭 프롬프트 필드 하이라이트 추가
- 변수가 적용된 프롬프트 줄 바꿈 적용

### Improved
- 프롬프트 모달 UI 개선
  - 화이트스페이스 처리 개선으로 줄바꿈 정상 적용
  - 확장 아이콘 위치 조정으로 접근성 향상
  - 변수 하이라이트 표시 일관성 유지

### Fixed
- 프롬프트 모달 UI 오류 수정
  - 원본 프롬프트와 변수 적용된 프롬프트 영역 레이아웃 개선
  - 확장 버튼 위치 일관성 확보
  - 변수 하이라이트 기능 통합
...
### Fixed
- 사용자 추가 탭 프롬프트 상세 UI 개선
  - 모달 최대 높이 조정으로 화면 활용도 향상
  - 프롬프트 내용 영역 높이 일관성 확보
  - 구분선 추가로 변수 영역과 내용 영역 구분 개선
- 사용자 추가 탭 프롬프트 추가/수정 오류 해결
  - 사용자 추가 프롬프트 ID 감지 로직 개선
  - 상태 관리 최적화로 프롬프트 수정 안정성 향상
...
### Improved
- 모달 영역 레이아웃 및 가독성 개선
  - 불필요한 테두리 제거로 깔끔한 UI 제공
  - 텍스트 크기 조정으로 가독성 향상
  - 프롬프트 내용 영역 확대로 더 많은 콘텐츠 표시
- 상태 관리 최적화
  - 사용자 추가 프롬프트 상태 관리 로직 개선
  - 프롬프트 항목 업데이트 함수 효율성 향상
...
### Fixed
- 사용자 추가 탭: 프롬프트 상세내용 오버레이 모달에서 메모 자동 저장 오류 해결
  - 사용자 추가 프롬프트 메모 업데이트 시 로컬 스토리지 저장 로직 연동
  - 사용자 추가 프롬프트 ID 감지 및 올바른 API 호출 경로 분기 처리
  - 프롬프트 상태 관리 로직 개선으로 사용자 추가 프롬프트 포함
