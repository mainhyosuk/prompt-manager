...
- **변수 관리 로직 전면 개편 및 안정화:**
    - 프롬프트 편집 모달 (`UserPromptEditModal`, `PromptAddEditModal`)에서 변수가 프롬프트 내용(`{{variable}}` 형식)에 의해서만 자동으로 인식되고 관리되도록 핵심 로직 변경.
    - 사용자가 직접 변수를 추가, 삭제, 수정하는 기능을 제거하고, 변수 목록(`VariableList.jsx`)은 읽기 전용으로 전환하여 인터페이스를 단순화했습니다.
    - 이를 통해 프롬프트 내용과 변수 데이터 간의 불일치로 인해 발생할 수 있는 다양한 오류를 원천적으로 방지하고 사용자 경험을 향상시켰습니다.
    - 백엔드 API에서 프롬프트 업데이트 시 빈 변수 배열(`[]`)을 올바르게 처리하여, 내용에서 모든 변수가 제거되면 데이터베이스에서도 해당 프롬프트의 모든 변수가 삭제되도록 수정했습니다.

- **폴더 관리 기능 강화:**
    - 폴더 삭제 시 해당 폴더에 속한 모든 프롬프트도 함께 삭제되도록 로직을 개선하여 데이터 정합성을 확보했습니다.
    - 폴더 목록 인터페이스에서 각 폴더별로 포함된 프롬프트의 개수가 정확하게 표시되도록 개선했습니다.

- **변수 기본값 처리 개선:**
    - 프롬프트 상세 보기 모달에서 변수의 기본값을 수정할 때, 텍스트 편집기에 해당 변수의 현재 저장된 기본값이 정확히 표시되도록 수정했습니다.
    - 변수 기본값 저장 로직의 안정성을 추가적으로 향상시켰습니다.

### 문제 원인 분석

- **변수 미삭제 문제:**
    - 프롬프트 내용에서 모든 변수를 제거하여 프론트엔드에서 백엔드로 빈 변수 배열 (`variables: []`)을 전송했을 때, 백엔드 API (`python/api/routes/prompt_routes.py`의 `update_prompt` 함수)의 조건문 (`if "variables" in data and data["variables"]`)으로 인해 해당 빈 배열이 올바르게 처리되지 않았습니다. 이로 인해 변수 업데이트 및 삭제 로직이 실행되지 않아 데이터베이스에 변수가 잔존하는 문제가 발생했습니다.
- **폴더 삭제 시 프롬프트 잔존 문제:**
    - 기존 폴더 삭제 로직 (`AppContext.jsx`의 `deleteFolder` 함수)이 폴더 엔티티만 삭제하고, 해당 폴더에 속한 프롬프트들의 `folder_id`를 정리하거나 프롬프트 자체를 삭제하는 처리가 누락되어 있었습니다.
- **변수 기본값 표시 오류:**
    - 프롬프트 상세 모달에서 변수의 기본값 수정 시, 변수의 실제 `default_value`가 아닌, 사용자가 이전에 임시로 입력했을 수 있는 다른 상태(`variableValues`)에서 값을 가져와 텍스트 에디터에 표시하여 혼란을 야기했습니다.

### 코드 변경 사항

- **`python/api/routes/prompt_routes.py`**:
    - `update_prompt` 함수: 변수 처리 조건문을 `if "variables" in data and data["variables"]:`에서 `if "variables" in data:`로 수정하여, `variables` 키가 존재하고 그 값이 빈 배열일 경우에도 관련 변수 삭제 로직이 정상적으로 수행되도록 변경.
- **`src/modals/UserPromptEditModal.jsx` & `src/modals/PromptAddEditModal.jsx`**:
    - `handleContentChange` 함수: 프롬프트 내용 변경 시 `extractVariablesFromContent` 유틸리티를 사용하여 변수를 추출하고, 이를 `variables` 상태로 설정하도록 로직 전면 개편.
    - 수동 변수 관리 UI 요소(추가/삭제 버튼, 입력 필드 등) 및 관련 상태 로직 제거.
    - `VariableList` 컴포넌트에 변수 수정을 위한 `setVariables` 등의 props 전달 중단.
    - 모달 초기 로드 시 `content`로부터 변수를 파생하여 `variables` 초기 상태를 설정하도록 변경.
    - 프롬프트 저장 시 API로 전송되는 `variables` 데이터는 현재 `content`에서 파생된 값만을 포함하도록 수정.
- **`src/components/variables/VariableList.jsx`**:
    - 변수 추가/삭제 버튼 및 관련 입력 필드 UI 제거.
    - 변수 이름 및 기본값 직접 변경을 위한 콜백 함수 props (`onAddVariable`, `onRemoveVariable` 등) 제거. 순수 표시용 컴포넌트로 전환.
- **`src/context/AppContext.jsx`**:
    - `deleteFolder` 함수: 폴더 삭제 API 호출 성공 후, 해당 폴더 ID를 가진 프롬프트들을 클라이언트 상태 및 로컬 스토리지에서 제거하는 로직 추가.
    - `loadFolders` 함수: 폴더 데이터 로드 시 프롬프트 개수 관련 정보를 함께 처리하도록 개선 (정확한 개수 표기를 위함).
- **`src/modals/PromptDetailModal.jsx`**:
    - `openTextEditor` 함수: 변수 기본값 편집용 텍스트 에디터에 값을 설정할 때, `variable.default_value`를 직접 사용하도록 수정.
    - `handleVariableDefaultValueChange` 함수: 변수 기본값 저장 시 업데이트된 값을 정확히 사용하도록 로직 수정.
- **`src/components/folder_system/FolderItem.jsx`**:
    - 폴더별 프롬프트 개수를 `folder.prompt_count` (또는 백엔드/컨텍스트에서 제공하는 유사한 속성)를 사용하여 정확히 표시하도록 수정.

### 기술적 개선

- **상태 관리 단순화 및 예측 가능성 증대:** 프롬프트 편집 과정에서 변수 상태를 항상 프롬프트 내용(`content`)으로부터 파생시킴으로써, 별도의 변수 관리 상태와 `content` 간의 복잡한 동기화 필요성을 제거했습니다. 이는 상태 관리 로직을 단순화하고 예측 가능성을 높여 유지보수성을 향상시킵니다.
- **데이터 일관성 및 무결성 강화:** 폴더 삭제 시 관련된 프롬프트들을 함께 처리하도록 변경하여 데이터 불일치 및孤立된 데이터 발생 가능성을 줄였습니다.
- **사용자 경험(UX) 개선:** 변수 관리 방식을 직관적으로 변경하고, 불필요한 UI 요소를 제거하여 사용자가 프롬프트 내용 작성에만 집중할 수 있도록 사용 편의성을 증대시켰습니다.
...
### 1. 휴지통 기능
프롬프트 관리의 안정성을 높이기 위한 휴지통 기능이 추가되었습니다.
- **삭제된 프롬프트 복구**: 실수로 삭제한 프롬프트를 쉽게 복원할 수 있습니다. 복구된 프롬프트는 기본 폴더로 이동합니다.
- **영구 삭제**: 더 이상 필요하지 않은 프롬프트는 휴지통에서 영구 삭제할 수 있습니다.
- **일괄 작업**: 다중 선택을 통해 여러 프롬프트를 한 번에 복구하거나 영구 삭제할 수 있습니다.
- **휴지통 접근**: 사이드바의 '삭제된 프롬프트' 메뉴를 통해 쉽게 휴지통에 접근할 수 있습니다.

### 2. 기본 폴더 기능
사용자 경험 개선을 위한 기본 폴더 기능이 추가되었습니다.
- **자동 프롬프트 정리**: 복구된 프롬프트는 자동으로 기본 폴더로 이동하여 쉽게 관리할 수 있습니다.
- **폴더 구조 최적화**: 기본 폴더가 '모든 프롬프트'와 '즐겨찾기' 다음 위치에 표시되어 빠르게 접근할 수 있습니다.

### 3. 라우팅 시스템 개선
애플리케이션의 내비게이션 시스템이 React Router 기반으로 전환되었습니다.
- **URL 기반 페이지 전환**: 각 페이지와 뷰가 고유 URL을 가지게 되어 직접 접근과 북마크가 가능합니다.
- **모달 일관성**: 모든 페이지에서 모달이 일관되게 표시되어 사용자 경험이 향상되었습니다.
- **폴더 이동 개선**: 폴더 클릭 시 페이지 간 이동이 자연스럽게 처리됩니다.

## 버그 수정
- **폴더 전환 오류**: 다른 페이지에서 폴더 클릭 시 발생하던 이동 오류가 해결되었습니다.
- **데이터베이스 무결성**: 프롬프트 삭제 및 복구 과정에서 데이터베이스 무결성을 보장하는 오류 처리가 강화되었습니다.

## 개발자 참고사항
- 데이터베이스 스키마에 `deletedAt` 및 `isDeleted` 필드가 추가되어 소프트 삭제를 지원합니다.
- DB 마이그레이션 스크립트를 실행하여 기존 데이터베이스를 업데이트해야 합니다.
- API 엔드포인트 추가: `/api/prompts/trash`, `/api/prompts/restore/:id`, `/api/prompts/permanent-delete/:id`
...
### 사용자 추가 프롬프트 데이터베이스 저장 기능 구현

**배경:** 기존에는 일반 프롬프트 상세 모달의 '사용자 추가' 탭에서 관리되던 프롬프트가 브라우저의 로컬 스토리지에 임시 저장되었습니다. 이로 인해 다른 환경이나 브라우저에서는 해당 데이터에 접근할 수 없었고, 데이터 유실의 위험이 있었습니다.

**개선 내용:**

*   **데이터 영속성 확보:** 사용자 추가 프롬프트를 생성, 수정, 삭제할 때 이제 중앙 데이터베이스에 직접 저장하고 관리합니다. 이를 통해 어떤 환경에서든 동일한 데이터에 접근하고 보존할 수 있습니다.
*   **백엔드 로직 통합:**
    *   `prompts` 테이블 스키마에 `is_user_prompt` (사용자 추가 여부), `user_id` (사용자 식별자), `parent_prompt_id` (원본이 되는 부모 프롬프트 ID) 컬럼을 추가했습니다.
    *   백엔드 API (`/api/prompts`)의 생성(POST), 조회(GET), 수정(PUT), 삭제(DELETE) 로직에 사용자 추가 프롬프트를 식별하고 처리하는 기능을 통합했습니다.
    *   GET 요청 시 `userId` 및 `parentId` 쿼리 파라미터를 통해 특정 사용자의 특정 부모에게 속한 프롬프트만 필터링하여 조회할 수 있도록 개선했습니다.
*   **프론트엔드 로직 변경:**
    *   `src/api/userPromptApi.js` 파일의 함수들이 로컬 스토리지 대신 백엔드 API를 호출하도록 전면 수정했습니다.
    *   `src/components/promptPanel/PromptPanel.jsx` 내 `UserAddedPromptsList` 컴포넌트가 DB에서 데이터를 로드하고, 생성/수정/삭제 시 목록을 올바르게 새로고침하도록 변경했습니다.
    *   `src/context/AppContext.jsx`의 상태 관리 로직을 개선하여, 사용자 프롬프트 데이터 변경 시 관련 UI 컴포넌트(상세 모달, 목록)의 상태가 즉시 동기화되도록 수정했습니다.

**기대 효과:**

*   사용자가 생성한 모든 프롬프트 데이터(일반, 사용자 추가)를 안전하게 보존하고 관리할 수 있습니다.
*   향후 사용자 계정 시스템 도입 시, 사용자별 프롬프트 관리를 위한 기반을 마련했습니다.
*   데이터 처리 로직을 백엔드로 일원화하여 코드 유지보수성을 향상시켰습니다.

## 수정된 버그

*   **사용자 추가 프롬프트 수정/삭제 오류:** 기존 로컬 스토리지 로직으로 인해 DB에 저장된 사용자 프롬프트를 수정/삭제할 수 없었던 문제를 해결했습니다.
*   **사용자 추가 프롬프트 목록 표시 오류:** 특정 부모 프롬프트와 관계없이 모든 사용자 추가 프롬프트가 표시되던 문제를 해결하고, 해당 부모에게 속한 프롬프트만 표시되도록 수정했습니다.
*   **UI 업데이트 지연 문제:** 사용자 추가 프롬프트의 메모나 변수를 업데이트했을 때, 상세 모달 UI에 변경 사항이 즉시 반영되지 않던 문제를 해결했습니다.

## 기술적 변경 사항

*   **데이터베이스 스키마 변경:** `prompts` 테이블에 `is_user_prompt`, `user_id`, `parent_prompt_id` 컬럼 추가 및 관련 마이그레이션 로직 구현 (`python/db/database.py`).
*   **API 엔드포인트 수정:**
    *   `POST /api/prompts`: `isUserPrompt`, `userId`, `parentId` 요청 필드 처리 로직 추가.
    *   `GET /api/prompts`: `userId`, `parentId` 쿼리 파라미터 필터링 기능 추가 및 `is_user_prompt`, `user_id`, `parent_prompt_id` 응답 필드 추가.
    *   `PUT /api/prompts/{id}`: `memo`, `variables` 업데이트 로직 수정 및 추가.
    *   `DELETE /api/prompts/{id}`: (기존 로직 활용).
*   **프론트엔드 API 모듈 수정:** `src/api/userPromptApi.js`의 모든 함수를 백엔드 API 호출 방식으로 변경.
*   **프론트엔드 컴포넌트 및 상태 관리 로직 수정:** `src/components/promptPanel/PromptPanel.jsx`, `src/context/AppContext.jsx`, `src/modals/UserPromptDetailModal.jsx` 등 관련 파일 수정.

## [v1.6] - 2025-04-13

### 주요 개선사항

- 홈 화면 프롬프트 카드 다중 선택 기능 추가:
    - 체크박스 UI 구현으로 직관적인 다중 선택 인터페이스 제공
    - 그리드 및 리스트 뷰 모두에서 일관된 다중 선택 경험 제공
    - 선택 항목 기반 일괄 작업 액션바 구현 (삭제, 폴더 이동)
    - 전체 선택/해제 토글 기능으로 대량 작업 효율성 향상
    - 선택 개수 실시간 표시로 작업 상태 시각화

- 프롬프트 카드 컴포넌트 재구성:
    - PromptItemCard를 BasePromptCard로 리팩토링하여 재사용성 강화
    - 패널별 프롬프트 카드 컴포넌트 일관성 확보
    - 다양한 컨텍스트에서 동일한 카드 디자인 언어 적용
    - 드래그앤드롭 관련 props 추가로 사용자 추가 리스트 기능 확장

### 문제 원인 분석

- 코드 중복 및 컴포넌트 일관성 부재:
    - 여러 탭과 패널에서 유사한 프롬프트 카드 컴포넌트 중복 구현으로 코드 비효율성 발생
    - 동일 기능의 카드가 다른 경로에 정의되어 유지보수 어려움 증가
    - 디자인 업데이트 시 여러 파일 수정 필요로 일관성 유지 어려움
    - 확장 기능 추가 시 모든 카드 컴포넌트 개별 수정 필요한 구조적 문제

- 대량 프롬프트 작업 비효율성:
    - 개별 프롬프트 조작 위주 UI로 대량 작업 불편
    - 단일 항목만 처리 가능한 작업 흐름으로 사용자 생산성 저하
    - 다중 항목 선택 및 관리 기능 부재로 폴더 정리 작업 복잡함

### 코드 변경 사항

- src/components/cards/BasePromptCard.jsx:
    - 컴포넌트 리팩토링:
        - PromptItemCard를 BasePromptCard로 이름 변경 및 중앙 컴포넌트 역할 구현
        - 기존 props 외에 드래그앤드롭 관련 props 추가 (onDragStart, onDragOver, onDragEnd, onDrop 등)
        - 재사용성 향상을 위한 유연한 props 구조 설계
        - 카드 타입과 상황별 조건부 렌더링 로직 개선

- src/components/prompts/PromptGrid.jsx:
    - 다중 선택 기능 구현:
        - 선택 상태 관리를 위한 props 추가 (selectedIds, onToggleSelect, isMultiSelectMode)
        - 체크박스 UI 구현 및 선택 상태에 따른 시각적 피드백 제공
        - 카드 클릭 시 다중 선택 모드 여부에 따른 동작 분기 처리
        - 카드 스타일을 선택 상태에 따라 동적으로 변경하는 클래스 구조 구현
        - 체크박스 클릭 이벤트 버블링 제어로 정확한 상호작용 보장

- src/components/prompts/PromptList.jsx:
    - 리스트 뷰 다중 선택 기능 구현:
        - 그리드 뷰와 동일한 선택 관련 props 및 기능 통합
        - 리스트 형태에 맞는 체크박스 UI 위치 및 스타일 구현
        - 항목 클릭 이벤트와 체크박스 클릭 이벤트 분리 처리
        - 다중 선택 모드에서 컨트롤 버튼 표시 제어 로직 구현
        - 가독성 향상을 위한 데이터 표시 최적화 (태그, 사용 횟수, 최근 사용 시간 등)

- src/hooks/useMultiSelect.js:
    - 다중 선택 기능을 위한 커스텀 훅 개발:
        - 선택된 ID 배열 상태 관리 (selectedIds)
        - 항목 선택/해제 토글 함수 구현 (toggleSelection)
        - 전체 선택/해제 기능 구현 (selectAll, deselectAll)
        - 현재 선택 상태 확인 기능 제공 (hasSelections, allSelected)
        - useCallback을 활용한 함수 참조 안정성 확보
        - 아이템 목록 변경에 따른 상태 자동 업데이트 로직 구현

- src/pages/Dashboard.jsx:
    - 다중 선택 모드 및 액션바 구현:
        - 다중 선택 모드 상태 관리 (isMultiSelectMode)
        - useMultiSelect 훅 통합으로 선택 항목 관리
        - 다중 선택 모드 전환 버튼 및 취소 기능 구현
        - 선택 항목 기반 일괄 작업 액션바 (상단 고정 위치)
        - 전체 선택/해제 토글 인터페이스 및 기능 구현
        - 선택 항목 개수 실시간 표시 기능
        - 일괄 삭제, 폴더 이동 액션 버튼 및 핸들러 함수 구현
        - 모드 전환 시 상태 초기화 로직 추가

- src/components/promptPanel/PromptPanel.jsx, CollectionsList.jsx, RecentPromptsList.jsx, SimilarPromptsList.jsx:
    - 컴포넌트 통합:
        - PromptItemCard 대신 BasePromptCard 사용으로 일관성 확보
        - 임포트 경로 수정 및 props 전달 구조 통일
        - 코드 중복 제거 및 컴포넌트 간 일관된 인터페이스 제공

- src/modals/PromptDetailModal.jsx:
    - UI 통합:
        - 유사한 프롬프트 섹션에서 PromptItemCard 대신 BasePromptCard 사용
        - props 전달 및 이벤트 핸들러 업데이트

### 기술적 개선

- 컴포넌트 구조 최적화:
    - 중앙화된 카드 컴포넌트(BasePromptCard)로 코드 중복 제거
    - 프롬프트 표시 로직 통합으로 일관성 확보 및 유지보수 용이
    - props 기반 확장성 강화로 새로운 기능 추가 용이
    - 클래스 구조화로 스타일 일관성 및 테마 적용 용이성 향상

- 사용자 경험 개선:
    - 다중 선택 UI로 대량 작업 효율성 향상
    - 체크박스 패턴 적용으로 직관적인 선택 메커니즘 제공
    - 그리드/리스트 뷰 간 일관된 다중 선택 경험 제공
    - 액션바를 통한 선택된 항목 집계 및 작업 옵션 시각화
    - 전체 선택/해제 기능으로 작업 효율성 향상

- 성능 및 코드 품질 향상:
    - useCallback 활용으로 함수 재생성 최소화
    - 커스텀 훅(useMultiSelect)으로 상태 관리 로직 분리 및 재사용성 증대
    - CSS 클래스 최적화로 렌더링 성능 향상
    - 조건부 렌더링 효율화로 DOM 최적화
    - 명확한 컴포넌트 역할 분담으로 코드 구조 개선
...
- 사용자 추가 탭 UI 및 상호작용 품질 향상:
    - 프롬프트 상세 모달 레이아웃 개선 및 화면 활용도 최적화
    - 프롬프트 내용 영역 높이 확대로 더 많은 콘텐츠 표시
    - 변수 영역과 내용 영역 간 구분선 추가로 시각적 구조화
    - 텍스트 크기 및 여백 조정으로 가독성 향상
    - 확대 보기 버튼 표시 로직 개선 (다른 모달 열릴 때 숨김 처리)

- 버전 관리 탭 오버레이 모달 UI 구조 개선:
    - 변수 저장 버튼 디자인 통일로 일관된 UX 제공
    - 텍스트 에디터 모달 상호작용 개선
    - 버튼 문구 명확화로 사용자 이해도 향상

### 문제 원인 분석

- 사용자 추가 탭 프롬프트 처리 오류:
    - AppContext의 openOverlayModal 함수에서 프롬프트 ID 타입 확인 로직 불완전
    - 프롬프트를 통합 목록에서 찾지 못할 경우 예외 처리 미흡
    - 사용자 추가 프롬프트 상태 관리 최적화 필요성 확인
    - userAddedPrompts 상태와 통합 prompts 상태 간 동기화 문제 발견

- UI 일관성 및 사용성 문제:
    - 모달 간 스타일 불일치로 사용자 경험 저하
    - 불필요한 테두리와 패딩으로 화면 공간 비효율적 사용
    - 프롬프트 내용 표시 영역 높이 제한으로 스크롤 과다 발생
    - 다양한 화면 크기에서 모달 표시 최적화 미흡

### 코드 변경 사항

- src/context/AppContext.jsx:
    - openOverlayModal 함수 개선:
        - 프롬프트 ID 타입 확인 로직 강화 (startsWith 메서드 활용)
        - 프롬프트를 찾지 못할 경우 전달된 객체 사용하도록 예외 처리 추가
        - 사용자 추가 프롬프트 여부 확인 로직을 is_user_added 플래그와 ID 패턴 기반으로 이중 확인
    - 상태 관리 최적화:
        - userAddedPrompts 상태 추가 및 초기화
        - loadData 함수에서 사용자 추가 프롬프트도 전역 상태에 저장하는 로직 추가
        - updatePromptItem 함수에서 userAddedPrompts 상태도 함께 업데이트하는 로직 추가

- src/modals/UserPromptDetailModal.jsx:
    - UI 개선:
        - 모달 최대 높이 조정 (h-[67vh] → max-h-[85vh])
        - 불필요한 테두리 및 배경색 제거로 깔끔한 디자인 구현
        - 프롬프트 내용 영역 높이 확대 (h-44 → h-80)
        - 변수 영역과 내용 영역 사이에 구분선 추가
        - 텍스트 크기 일관성 확보 (text-xs → text-sm)
    - 기능 개선:
        - 확대 보기 버튼 표시 조건 변경으로 다른 모달 열릴 때 버튼 중복 방지
        - 내용 영역에 이중 스크롤 구조 적용으로 레이아웃 안정성 향상

- src/modals/VersionDetailModal.jsx:
    - UI 통일성 확보:
        - UserPromptDetailModal과 동일한 레이아웃 및 스타일 적용
        - 변수 저장 버튼 디자인 현대화 및 사용성 개선
    - 기능 개선:
        - handleSaveVariableValue 함수를 useCallback으로 최적화하여 성능 향상
        - 변수 값 변경 시 조건부 업데이트로 불필요한 API 호출 방지
        - 텍스트 에디터 모달 버튼 명확화 ("적용" → "적용 (현재만)")

- src/components/promptPanel/PromptPanel.jsx:
    - 코드 구조 개선:
        - handlePromptAdded 함수 위치 조정으로 관련 기능 그룹화
        - 주석 설명 개선으로 코드 가독성 향상

### 기술적 개선

- UI/UX 품질 향상:
    - 모달 레이아웃 최적화로 더 많은 콘텐츠 표시
    - 일관된 디자인 언어 적용으로 사용자 혼란 감소
    - 스크롤 구조 개선으로 내용 탐색 용이성 증대
    - 텍스트 크기 및 여백 최적화로 가독성 향상

- 코드 품질 및 유지보수성 개선:
    - useCallback 사용으로 함수 참조 안정성 및 성능 향상
    - 조건부 렌더링 최적화로 불필요한 UI 요소 방지
    - 예외 처리 강화로 애플리케이션 안정성 증대
    - 중복 코드 제거 및 코드 구조화로 유지보수 용이성 향상
