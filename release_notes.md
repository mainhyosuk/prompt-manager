...
- 프롬프트 카드 UI 개선:
    - 프롬프트 내용을 최대 4줄까지 표시하고 스크롤 기능 추가
    - 높이 고정으로 카드 크기 일관성 유지 (내용이 적어도 동일한 크기)
    - 사용 횟수 정보 추가 표시 및 아이콘 적용
    - 스크롤바 스타일 개선으로 시각적 일관성 강화
    - 프롬프트 내용 영역에 배경색 및 패딩 적용으로 가독성 향상

### 코드 변경 사항

- src/components/prompts/PromptCard.jsx:
    - 프롬프트 미리보기 개선:
        - 내용을 100자로 제한하는 로직 제거
        - 고정 높이(h-24)와 스크롤(overflow-y-auto) 적용
        - 내용 영역에 스타일 적용(배경색, 패딩, 둥근 모서리)
        - whitespace-pre-wrap 적용으로 줄바꿈 유지
    
    - 메타데이터 표시 개선:
        - 사용 횟수 정보 추가 (User 아이콘과 함께 표시)
        - 하단 메타데이터 레이아웃 개선(Flexbox 활용)
        - 마지막 사용 시간 표시 유지
        - 폴더 위치 표시 유지

- src/styles/index.css:
    - 스크롤바 스타일 개선:
        - 작고 세련된 4px 너비의 스크롤바 구현
        - 스크롤바 트랙 및 썸(thumb) 스타일 커스터마이징
        - 호버 상태에서의 스크롤바 인터랙션 개선
        - 다크 모드에서의 스크롤바 스타일 지원

### 기술적 개선

- 사용자 경험 향상:
    - 일관된 카드 크기로 UI의 시각적 안정성 확보
    - 더 많은 프롬프트 내용을 스크롤로 확인 가능
    - 사용 횟수 정보 제공으로 자주 사용하는 프롬프트 식별 용이
    - 커스텀 스크롤바 스타일로 세련된 UI 제공

- 접근성 개선:
    - 내용 영역에 시각적 구분 추가로 가독성 향상
    - 스크롤 기능으로 프롬프트 전체 내용 접근 용이
    - 다크 모드 지원으로 다양한 환경에서 일관된 경험 제공
